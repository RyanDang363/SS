...
Usage:
    python -m video_rag.validate <path> --type <artifact_type>

JSON artifacts validate a single record. JSONL artifacts validate every
non-blank line. ``transcript_segments`` also enforces file-level ordering rules.
...
def _validate_jsonl(path: Path, artifact_type: str) -> tuple[bool, str]:
    model = JSONL_SCHEMAS[artifact_type]
    try:
        ...
        return False, f"FAIL  {path}: {e.__class__.__name__}: {e}"
    return True, f"OK    {path} ({artifact_type}, {len(records)} records)"
...
def validate_file(path: Path, artifact_type: str) -> tuple[bool, str]:
    ...
    if artifact_type in JSONL_SCHEMAS:
        return _validate_jsonl(path, artifact_type)
